...
import oead
from sms_bin_editor.utils.iohelper import (read_string, read_ubyte,
...
class ResourceAttribute(IntEnum):
    FILE = 0x01
    DIRECTORY = 0x02
    COMPRESSED = 0x04
...
class Directory(object):
    ...
    def from_node(cls, f, _name, stringtable_offset, globalentryoffset, dataoffset, nodelist, currentnodeindex, parents=None):
        ...
        for i in range(entrycount):
            ...
            fileid, hashcode, flags, padbyte, nameoffset, filedataoffset, datasize, padding = unpack(
                ">HHBBHIII", fileentry_data)
            ...
            # fileid == 0xFFFF: # entry is a sub directory
            if (flags & ResourceAttribute.DIRECTORY) != 0 and not (flags & ResourceAttribute.FILE):
                ...
                subdir = Directory.from_node(
                    f, name, stringtable_offset, globalentryoffset, dataoffset, nodelist, nodeindex, parents=newparents)
                subdir.parent = newdir

                newdir.subdirs[subdir.name] = subdir

            else:  # entry is a file
                if flags & ResourceAttribute.COMPRESSED:
                    print("File is compressed")
                if flags & ResourceAttribute.YAZ0_COMPRESSED:
                    print("File is yaz0 compressed")
                f.seek(offset)
                file = File.from_fileentry(
                    f, stringtable_offset, dataoffset, fileid, hashcode, flags, nameoffset, filedataoffset, datasize)
    ...
    def __getitem__(self, path):
        ...
        if rest is None or rest.strip() == "":
            ...
            raise RuntimeError(
                "File", name, "is a directory in path", path, "which should not happen!")
        else:
            ...
            if isinstance(name, File):
                if name in self.subdirs:
                    raise FileExistsError(
                        "Cannot add file, '{}' already exists as a directory".format(path))

                self.files[name] = entry
            elif isinstance(name, Directory):
                if name in self.files:
                    raise FileExistsError(
                        "Cannot add directory, '{}' already exists as a file".format(path))

                self.subdirs[name] = entry
            else:
                raise TypeError(
                    "Entry should be of type File or Directory but is type {}".format(type(entry)))
...
class File(BytesIO):
    ...
    def is_yaz0_compressed(self):
        if self._flags & ResourceAttribute.COMPRESSED and not self._flags & ResourceAttribute.YAZ0_COMPRESSED:
            print(
                "Warning, file {0} is compressed but not with yaz0!".format(self.name))
    ...
    def dump(self, f):
        if self.is_yaz0_compressed():
            f.write(oead.yaz0.decompress(self))
        else:
...
class Archive(object):
    ...
    def from_file(cls, f):
        ...
        if header == b"Yaz0":
            ...
            start = time.time()
            f.seek(0)
            BytesIO(oead.yaz0.decompress(f))
            f.seek(0)
        ...
        if header == b"RARC":
            ...
            raise RuntimeError(
                f"Unknown file header: {header} should be Yaz0 or RARC")
        ...
        for i in range(node_count):
            ...
            nameoffset, unknown, entrycount, entryoffset = unpack(
                ">IHHI", nodedata)

            if i == 0:
                dir_name = stringtable_get_name(
                    f, stringtable_offset, nameoffset)
            else:
                dir_name = None
        ...
        newarc.root = Directory.from_node(
            f, rootfoldername, stringtable_offset, file_entry_offset, data_offset, nodes, 0)
    ...
    def __setitem__(self, path, entry):
        ...
        if rest is None or rest.strip() == "":
            if dirname != self.root.name:
                raise RuntimeError(
                    "Cannot have more than one directory in the root.")
            elif isinstance(entry, Directory):
                self.root = entry
            else:
                raise TypeError(
                    "Root entry should be of type directory but is type '{}'".format(type(entry)))
        else:
    ...
    def write_arc_compressed(self, f, compression_settings, filelisting=None, maxindex=0):
        ...
        if compression_settings.yaz0_fast:
            temp.write(oead.yaz0.compress(f))
    ...
    def write_arc(self, f, compression_settings, filelisting=None, maxindex=0):
        ...
        for i, dirinfo in enumerate(self.root.walk()):
            ...
            # Each directory has two special entries being the current and the parent directories
            first_file_entry_index += entrycount + 2
        ...
        for dir in dirlist:
            ...
            for filepath, file in files:
                ...
                if filelisting is not None:
                    ...
                    write_uint16(f, fileid)
                filename = file.name
                write_uint16(f, _hash_name(filename))
                #print("Writing filemeta", str(filemeta))
                write_ubyte(f, filemeta.to_flags())
                write_ubyte(f, 0)  # padding
            ...
            for subdirname, subdir in chain(specialdirs, dir.subdirs.items()):
                ...
                write_uint16(f, _hash_name(subdirname))
...
if __name__ == "__main__":
    ...
    compression_setting = CompressionSetting(
        args.yaz0fast, args.wszst, args.wszst_comprlevel)
    ...
    if dir2arc:
        ...
        if inputdir is None:
            raise RuntimeError(
                "Directory {0} contains no folders! Exactly one folder should exist.".format(inputpath))
        ...
        with open(outputpath, "wb") as f:
            if args.yaz0fast or args.wszst:
                archive.write_arc_compressed(
                    f, compression_setting, filelisting, maxindex)
            else:
                archive.write_arc(f, compression_setting,
                                  filelisting, maxindex)
        print("Done")
    else:
