...
from juniors_toolbox.utils.j3d.anim.general_animation import (AnimComponent,
                                                              BasicAnimation,
                                                              combine_dicts,
                                                              find_sequence, get_bone_transforms,
                                                              make_tangents,
                                                              write_values)
...
class BoneAnim():
    ...
    def get_empty_bone_anim(cls, name, values=[1.0, 1.0, 1.0, 0, 0, 0, 0, 0, 0], angle_scale=0):
        ...
        new_anim.add_scale("X", AnimComponent(0, values[0]))
        new_anim.add_scale("Y", AnimComponent(0, values[1]))
        new_anim.add_scale("Z", AnimComponent(0, values[2]))
        new_anim.add_rotation("X", AnimComponent(0, values[3]))
        new_anim.add_rotation("Y", AnimComponent(0, values[4]))
        new_anim.add_rotation("Z", AnimComponent(0, values[5]))
        new_anim.add_translation("X", AnimComponent(0, values[6]))
        new_anim.add_translation("Y", AnimComponent(0, values[7]))
        new_anim.add_translation("Z", AnimComponent(0, values[8]))

        return new_anim


class BCK(BasicAnimation):
    ...
    def from_bytes(cls, data: BinaryIO, *args: VariadicArgs, **kwargs: VariadicKwargs):
        ...
        svr_data = data.read(12)
        sound = read_uint32(data)
        #print("sound " + str(sound) )

        ank_start = data.tell()
        ank_magic = data.read(4)  # ank1
        ank_size = read_uint32(data)
        ...
        bone_count = read_uint16(data)
        scale_count = read_uint16(data)
        rotation_count = read_uint16(data)
        trans_count = read_uint16(data)

        bone_offset = read_uint32(data) + ank_start
        scale_offset = read_uint32(data) + ank_start
        rotation_offset = read_uint32(data) + ank_start
        trans_offset = read_uint32(data) + ank_start
        ...
        # Read translations
        trans = []
        data.seek(trans_offset)
        for i in range(trans_count):
            trans.append(read_float(data))

        tangent_type = 0

        data.seek(bone_offset)
        for i in range(bone_count):
            ...
            z_scale, z_rot, z_trans = values[18:
                                             21], values[21:24], values[24:27]

            bone_animation = BoneAnim()
        ...
        if sound != 0xffffffff:
            ...
            for i in range(num_entries):
                ...
                sound_id = read_uint32(data)
                start_time = read_float(data)
                end_time = read_float(data)
                coarse_pitch = read_float(data)
                flags = read_uint32(data)
                volume = read_ubyte(data)
                fine_pitch = read_ubyte(data)
                loop_count = read_ubyte(data)
                pan = read_ubyte(data)
                unk_byte = read_ubyte(data)

                data.read(0x7)

                entry = SoundEntry(sound_id, start_time, end_time, coarse_pitch,
                                   flags, volume, fine_pitch, loop_count, pan, unk_byte)
        ...
        while (i < len(lines)):
            ...
            while(new_bone_name == current_bone):
                ...
                while(not "}" in lines[i]):
                    ...
                    new_entry = AnimComponent(int(values[0]), float(values[1]))
        ...
        for anim in bck.animations:
            for axis in {"X", "Y", "Z"}:
                if len(anim.scale[axis]) == 0:
                    new_entry = AnimComponent(0, 1.0)
                    anim.add_scale(axis, new_entry)
                if len(anim.rotation[axis]) == 0:
                    new_entry = AnimComponent(0, 0)
    ...
    def from_blender_bvh(cls, filepath):
        ...
        for i in range(len(all_values)):
            for j in range(len(all_values[i])):
                all_values[i][j] = AnimComponent(j, all_values[i][j])
        #print( all_values)
        ...
        while hierachy_index < motion_line:
            ...
            curr_bone.add_scale("X", AnimComponent(0, 1.0))
            curr_bone.add_scale("Y", AnimComponent(0, 1.0))
            curr_bone.add_scale("Z", AnimComponent(0, 1.0))
                ...
                if channels.find(axis) != -1:
                    ...
                    if axis.find("position") != -1:
                        curr_bone.add_translation(
                            axis[0], AnimComponent(0, 0.0))
                    else:
                        curr_bone.add_rotation(
                            axis[0], j3d.AnimComponent(0, 0.0))
    ...
    def get_loading_information(self):
        info = []
        info.append(["Loop Mode:", j3d.loop_mode[self.loop_mode], "Angle Scale:",
                    self.anglescale, "Duration:", self.duration, "Tan Type:", self.tan_type])
        ...
        for anim in self.animations:
            ...
            for j in range(len(things)):    # for each srt xyz component
                ...
                keyframes_dictionary = combine_dicts(
    ...
    def empty_table(cls, created):
        info = []
        info.append(["Loop_mode", "", "Angle Scale:", "",
                    "Duration:", created[3], "Tan Type:", j3d.tan_type[1]])
        info.append(["Bone Name", "Tangent Interpolation", "Component"])
        ...
        for i in range(int(created[1])):
            ...
            things = [["", "SSSS", "Scale Y:"], ["", "SSSS", "Scale Z:"],
                      ["", "LLLL", "Rotation X:"], ["", "LLLL",
                                                    "Rotation Y:"], ["", "LLLL", "Rotation Z:"],
    ...
    def from_table(cls, f, info, sound_data=None):
        ...
        for i in range(int(len(info) / 9)):  # for each bone
            ...
            for j in range(9):  # for each of thing in scale/rot/trans x/y/z/
                ...
                for k in range(3, len(info[line + j])):  # for each keyframe
                    if info[line + j][k] != "":
                        try:
                            comp = AnimComponent(
                                keyframes[k-3], float(info[line + j][k]))

                        except:
                            comp = AnimComponent(
                                bck.duration, float(info[line + j][k]))
            ...
            for j in range(9):
                ...
                xyz = xyz[j % 3: j % 3 + 1]
                if j < 3:
                    current_anim.scale[xyz] = make_tangents(
                        current_anim.scale[xyz], current_anim.tan_inter[j])
                if j < 6:
                    current_anim.rotation[xyz] = make_tangents(
                        current_anim.rotation[xyz], current_anim.tan_inter[j])
                else:
                    current_anim.translation[xyz] = make_tangents(
                        current_anim.translation[xyz], current_anim.tan_inter[j])
    ...
    def to_bytes(self) -> bytes:
        ...
        count_offset = f.tell()
        # Placeholder for scale, rotation and translation count
        f.write(b"1+1=11")
        ...
        for anim in self.animations:
            for axis in "XYZ":
                # Scale count for this animation
                write_uint16(f, len(anim.scale[axis]))
                # Offset into scales
                write_uint16(f, anim._scale_offsets[axis])
                # Tangent type, 0 = only TangentIn; 1 = TangentIn and TangentOut
                write_uint16(f, self.tan_type)

                # Rotation count for this animation
                write_uint16(f, len(anim.rotation[axis]))
                # Offset into rotations
                write_uint16(f, anim._rot_offsets[axis])
                # Tangent type, 0 = only TangentIn; 1 = TangentIn and TangentOut
                write_uint16(f, self.tan_type)

                # Translation count for this animation
                write_uint16(f, len(anim.translation[axis]))
        ...
        write_uint32(f, total_size - ank1_start)
        ...
        write_uint32(f, bone_anim_start - ank1_start)
        write_uint32(f, scale_start - ank1_start)
        write_uint32(f, rotations_start - ank1_start)
        write_uint32(f, translations_start - ank1_start)
        ...
        if self.sound is not None and self.sound:
            ...
            f.seek(total_size)
            write_uint16(f, len(self.sound))
            write_ubyte(f,  0xc)
            f.write(b"\x00"*5)

            for entry in self.sound:
                write_uint32(f, int(entry.sound_id))
                write_float(f, float(entry.start_time))
                write_float(f, float(entry.end_time))
                write_float(f, float(entry.coarse_pitch))
                write_uint32(f, int(entry.flags))
                write_ubyte(f, int(entry.volume))
                write_ubyte(f, int(entry.fine_pitch))
                write_ubyte(f, int(entry.loop_count))
                write_ubyte(f, int(entry.pan))
                write_ubyte(f, int(entry.unk_byte))
                f.write(b"\x00"*0x7)
        ...
        with open(filepath, "w") as f:
            ...
            for i in range(len(self.animations)):
                ...
                if len(anim.scale["X"]) > 0:
                    write_single_comp(
                        f, "scaleX", child, j, anim.scale["X"], anim.tan_inter[0], bones[i])
                    j += 1
                if len(anim.scale["Y"]) > 0:
                    write_single_comp(
                        f, "scaleY", child, j, anim.scale["Y"], anim.tan_inter[1], bones[i])
                    j += 1
                if len(anim.scale["Z"]) > 0:
                    write_single_comp(
                        f, "scaleZ", child, j, anim.scale["Z"], anim.tan_inter[2], bones[i])
                    j += 1
                if len(anim.rotation["X"]) > 0:
                    write_single_comp(
                        f, "rotateX", child, j, anim.rotation["X"], anim.tan_inter[3], bones[i])
                    j += 1
                if len(anim.rotation["Y"]) > 0:
                    write_single_comp(
                        f, "rotateY", child, j, anim.rotation["Y"], anim.tan_inter[4], bones[i])
                    j += 1
                if len(anim.rotation["Z"]) > 0:
                    write_single_comp(
                        f, "rotateZ", child, j, anim.rotation["Z"], anim.tan_inter[5], bones[i])
                    j += 1
                if len(anim.translation["X"]) > 0:
                    write_single_comp(
                        f, "translateX", child, j, anim.translation["X"], anim.tan_inter[6], bones[i])
                    j += 1
                if len(anim.translation["Y"]) > 0:
                    write_single_comp(
                        f, "translateY", child, j, anim.translation["Y"], anim.tan_inter[7], bones[i])
                    j += 1
                if len(anim.translation["Z"]) > 0:
                    write_single_comp(
                        f, "translateZ", child, j, anim.translation["Z"], anim.tan_inter[8], bones[i])
                    j += 1
    ...
    def match_bmd(cls, info, strings, filepath=None):
        ...
        # print(bone_names)
        BasicAnimation.match_bmd(bck, strings)
        ...
        jnt_vals = get_bone_transforms(filepath)
        ...
        while i < len(strings) and i < len(bck.animations):
            ...
            if anim.name != strings[i]:
                print(strings[i])
                bck.animations.insert(
                    i, BoneAnim.get_empty_bone_anim(strings[i], jnt_vals[i]))
            i += 1
...
class SoundEntry:
    ...
    def read_sound_data(cls, filepath):
        with open(filepath, "rb") as f:
            ...
            if header == b"Yaz0":
                f = BytesIO(oead.yaz0.decompress(f))
            ...
            for i in range(num_entries):
                ...
                sound_id = read_uint32(f)
                start_time = read_float(f)
                end_time = read_float(f)
                coarse_pitch = read_float(f)
                flags = read_uint32(f)
                volume = read_ubyte(f)
                fine_pitch = read_ubyte(f)
                loop_count = read_ubyte(f)
                pan = read_ubyte(f)
                unk_byte = read_ubyte(f)
...
def write_single_comp(f, comp, children, j, array, tan_inter, name):
    f.write("anim " + comp[0:-1] + "." + comp + " " + comp +
            " " + name + " 0 " + str(children) + " " + str(j) + ";\n")
    output_type = "linear"
    if len(comp) == 7:
        output_type = "angular"
    f.write("animData {\n\tinput time;\n\toutput "+output_type +
    ...
    for frame in array:
        f.write("\t\t" + str(frame.time) + " " + str(frame.value) +
                " " + tan_inter + " " + tan_inter + " 1 1 0;\n")
