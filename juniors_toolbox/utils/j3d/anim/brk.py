...
class ColorAnimation():
    def __init__(self, index=0, name="BRKColorAnim", unknown=0):
        self._index = index
    ...
    def from_brk(cls, f, name, index, rgba_arrays):
        ...
        for i, comp in enumerate(("R", "G", "B", "A")):
            ...
            for j in range(count):
                animcomp = AnimComponent.from_array(
                    offset, j, count, rgba_arrays[i], tangent_type)
...
class BRK(BasicAnimation):
    ...
    def get_loading_information(self):
        ...
        info.append(["Loop Mode:", j3d.loop_mode[self.loop_mode],
                    "Duration:", self.duration, "Tan Type:", j3d.tan_type[1]])
    ...
    def empty_table(cls, created):
        ...
        info.append(["Loop_mode", "", "Duration:", created[3],
                    "Tangent Type:", j3d.tan_type[1]])
        info.append(["Material Name", "Color Index", "Channel",
                    "Frame 0", "Frame " + str(created[3])])
    ...
    def from_table(cls, f, info):
        ...
        for i in range(0, int(len(info) / 4)):
            ...
            color_anim = ColorAnimation(
                i, info[curr_line][0], int(info[curr_line][1]))
    ...
    def to_bytes(self) -> bytes:
        ...
        for animtype, animations in (
            ...
            for colorcomp in ("R", "G", "B", "A"):
                ...
                for anim in animations:
                    ...
                    offset = find_sequence(
                        all_values[animtype][colorcomp], sequence)
        ...
        for anim in self.register_animations:
            for comp in ("R", "G", "B", "A"):
                ...
                write_uint16(f, len(anim.component[comp]))
                # Offset into scales
                write_uint16(f, anim._component_offsets[comp])
