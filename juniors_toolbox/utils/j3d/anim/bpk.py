...
class ColorAnimation():
    def __init__(self, index=0, name="BPKColorAnim"):
        self._index = index
    ...
    def from_bpk(cls, f, name, index, rgba_arrays):
        ...
        for i, comp in enumerate(("R", "G", "B", "A")):
            ...
            for j in range(count):
                animcomp = j3d.AnimComponent.from_array(
...
class BPK(BasicAnimation):
    ...
    def get_loading_information(self):
        ...
        info = []
        info.append(["Loop Mode:", j3d.loop_mode[self.loop_mode], "Duration:",
                ...
                for k in range(2, len(info[curr_line + j])):
                    if info[curr_line + j][k] != "":
                        anim_comp = j3d.AnimComponent(
                            keyframes[k - 2], int(info[curr_line + j][k]))
